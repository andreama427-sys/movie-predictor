...
# ── Helper functions (identical to your notebook) ─────────────────────────────
...
# ── Load data + train models (cached — only runs once) ────────────────────────

@st.cache_resource(show_spinner="Loading data and training models… this takes about 30 seconds")
def load_and_train():
    movies = pd.read_csv("tmdb_5000_movies.csv", engine="python", on_bad_lines="skip")

    # The original credits file was too large for GitHub's browser upload limit,
    # so it was split into 3 smaller CSV files. This reads all 3 parts and
    # combines them back into one credits dataframe before merging with movies.
    credits_part1 = pd.read_csv("tmdb_5000_credits_part1.csv", engine="python", on_bad_lines="skip")
    credits_part2 = pd.read_csv("tmdb_5000_credits_part2.csv", engine="python", on_bad_lines="skip")
    credits_part3 = pd.read_csv("tmdb_5000_credits_part3.csv", engine="python", on_bad_lines="skip")

    credits = pd.concat(
        [credits_part1, credits_part2, credits_part3],
        ignore_index=True
    )
    ...
    mdf = df.dropna(subset=["budget", "revenue", "runtime", "popularity",
                              "vote_average", "vote_count", "release_year", "release_month"])
    mdf = mdf[
        (mdf["budget"] > 0) & (mdf["revenue"] > 0) &
        (mdf["runtime"] > 0) & (mdf["vote_count"] >= 25) &
        (mdf["main_genre"] != "Unknown")
    ].copy()

    # Build performance score + labels
    ...
    X = pd.get_dummies(mdf[features].copy(), columns=["main_genre"], drop_first=False).fillna(0)
    y = mdf["performance_label"]
...
# ── Prediction + explanation (your notebook Section 17 logic) ─────────────────
...
def build_explanation(pred, conf, budget, runtime, overview):
    ...
    if budget >= 100_000_000:
        reasons.append("a large budget, which supports stronger production value and marketing reach")
    ...
    return (
        ...
        f"The NLP analysis of the plot summary found a sentiment score of **{sent:+.3f}**, "
